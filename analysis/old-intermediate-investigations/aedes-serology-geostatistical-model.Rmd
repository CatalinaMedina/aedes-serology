...
```{r Response histogram, eval = FALSE}
aedes_red %>% 
  filter(visit != "visit5") %>% 
...
main_fit_plot_df <- main_fit_df %>% 
  ...
  add_row(pred_names = "Insecticide used [ref: No]", .before = 4) %>% 
  add_row(pred_names = "Larvicide used [ref: No]", .before = 6) %>% 
  add_row(pred_names = "Modal land type [ref: Urban]", .before = 10) %>%
  add_row(pred_names = "Wet season [ref: No]", .before = 14)

main_fit_plot_df$pred_names[2]  <- "     Male"
main_fit_plot_df$pred_names[5]  <- "     Yes"
main_fit_plot_df$pred_names[7]  <- "     Yes "
main_fit_plot_df$pred_names[11] <- "     Rice paddy"
main_fit_plot_df$pred_names[12] <- "     Cropland"
main_fit_plot_df$pred_names[15] <- "     Yes  "

main_fit_plot_df <- main_fit_plot_df %>% 
