...
## Kriging
```{r}
aedes_v1 <- filter(aedes_red, wet_season == "no")


aedes_v1_spdf <- SpatialPointsDataFrame(
  coords = select(aedes_v1, lon, lat),
  data = select(aedes_v1, distance_to_river_scaled)
)

# aedes_dry_rast <- aedes_dry
# coordinates(aedes_dry_rast) <- ~ lon + lat
# gridded(aedes_dry_rast) <- TRUE

krig_fit_v1 <- lgm(
  log_saliva_elisa ~ distance_to_river_scaled,
  data = aedes_v1_spdf,
  grid = 120, 
  covariates = NULL,
  Ntrials = 2
