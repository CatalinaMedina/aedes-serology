...
sat_col_ex <- df_og[, sat_cols]

new_row <- 1

for(i in 1:nrow(df_og)) {
  for(j in 0:(length(sat_cols) - 1)) {
    new_df[new_row + j, non_sat_cols] <- df_og[i, non_sat_cols]
    col_name_ex <- str_split(colnames(sat_col_ex)[j + 1], pattern = "_")
    ...
    new_df[new_row + j, "sat_var_ind"] <- sat_col_ex[j + 1]
  }
