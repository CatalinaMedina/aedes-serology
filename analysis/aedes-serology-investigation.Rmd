...
params:
  ...
  rerun_fit_young1_bayes: FALSE
  rerun_fit_young1_bayes_no_visit5: FALSE
  rerun_fit_young2_bayes: FALSE
  rerun_fit_school1: FALSE
  rerun_fit_school1_no5: FALSE
  rerun_fit_school2: FALSE
  rerun_fit_young3: FALSE
  rerun_fit_school3: FALSE
  fit_main1: FALSE
  fit_simple: FALSE
---
...
aedes_red <- aedes_red %>% 
  mutate(lat_v_lon = lat * lon) %>% 
  mutate(lat_v_lon_s = (lat_v_lon - mean(lat_v_lon)) / sd(lat_v_lon))
...
2nd Cambodia meeting notes:  

*	Replace visit indicators with one predictor for wet season and then a predictor for year.  
*	Add predictor for living within “” meters of the river.  
*	Add larval count matching closets larval count.  

*	We should have two models:  
    + Model aedes levels for children 2-4. This model will have an indicator for school attendance. We are unsure how to account for the fact that children can begin school at different ages and we do not have repeat data collection for school so we do not know if a child began school after the first visit.  
    + Model aedes levels for children in school (ages 5-9). This model will have environmental variables for school and home.

*	There was a change in the person processing the aedes ELISA data for visit 5 (and a change in location mosquitoes used in process were from [US vs Cambodia]).  
*	Negative effect for home environment in school children model may be because of extra preventative measures  
* Try interactions between wet season and toilets, water containers, larvicide, and insecticide
...
gen_post_plots <- function(fit){
  ...
  fit_post_area2 <- mcmc_areas(
    fit,
    regex_pars = c("gender", "age", "school", "river"),
    prob = 0.95, prob_outer = 0.99
  ) +
    scale_y_discrete(labels = c(
      "genderMale" = "Sex(male)",
  ...
  fit_post_area4 <- mcmc_areas(fit, regex_pars = c("NFI250_home", "EVI250_home", "year"),
    prob = 0.95, prob_outer = 0.99)
  
  
  grid.arrange(
    fit_post_area1, fit_post_area2, textGrob(" "), fit_post_area4, 
    nrow = 2
  )
}
```
...
if(params$rerun_fit_young1_bayes) {
  fit_young1_bayes <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
  ...
  saveRDS(fit_young1_bayes, file = here("output", "fit-young1-bayes.rds"))
  
} else { 
  fit_young1_bayes <- readRDS(file = here("output", "fit-young1-bayes.rds")) 
}
```
...
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_young1_bayes)
```
\elandscape
...
```{r fit-young1-bayes-no-visit5}
aedes_red_young5 <- aedes_red[(aedes_red$age <= 4) & (aedes_red$visit != "visit5"),]

if(params$rerun_fit_young1_bayes_no_visit5) {
  fit_young1_bayes_no_visit5 <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented3 + in_school + distance_to_river_scaled +
      num_toilets_in_home + num_domestic_water_containers_in_home + 
      insecticide_spray + larvicide_applied_to_water + 
      NFI250_home + EVI250_home + 
      wet_season + visit_year,
    data = aedes_red_young5,
    seed = seed_val
  )
  
  saveRDS(fit_young1_bayes_no_visit5, file = here("output", "fit-young1-bayes-no-visit5.rds"))
  
} else { 
  fit_young1_bayes_no_visit5 <- readRDS(file = here("output", "fit-young1-bayes-no-visit5.rds")) 
}
```

\blandscape
Young model without interactions (without visit 5)
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_young1_bayes_no_visit5)
```
\elandscape

Model fit: Young children with interactions  
Updated model for log saliva ELISA value of individual $i$ at measurement $j$ ($i=1,...,726$, $j=1,...,5$), for Cambodian children ages 2-4.
\begin{equation*}
	\begin{split}
		\log(\text{Saliva ELISA value})_{ij} &= b_{0i} + \beta_{0} + \beta_{\text{Male}}\text{Male}_i \\
		...
		&+ \beta_{\text{NFI250 home}}\text{Home flooding}_{ij} + \beta_{\text{EVI250 home}}\text{Home vegetation}_{ij} \\
		&+ \beta_{\text{Wet season}}\text{Wet season}_{ij} + \beta_{\text{Year}}\text{Year}_{ij}\\
		&+\beta_{\text{Toilets Int.}}\text{Toilets}_{ij}\times\text{Wet season}_{ij} + \beta_{\text{Water Int.}}\text{Water containers}_{ij}\times\text{Wet season}_{ij} + \beta_{\text{Insecticide Int.}}\text{Insecticide}_{ij}\times\text{Wet season}_{ij} + \beta_{\text{Larvicide Int.}}\text{Larvicide}_{ij}\times\text{Wet season}_{ij} \\
	\end{split}
\end{equation*}
...
gen_post_plots <- function(fit){
  ...
  fit_post_area2 <- mcmc_areas(
    ...
    scale_y_discrete(labels = c(
      ...
      "age_centered3" = "Age",
      "in_schoolyes" = "In School(yes)",
      "distance_to_river_scaled" = "Distance from river"
    ))
  
  fit_post_area3 <- mcmc_areas(
    fit, 
    regex_pars = c(
      "num_toilets_in_home:wet_seasonyes", 
      "num_domestic_water_containers_in_home:wet_season",
      "wet_seasonyes:larvicide_applied_to_waterYes",
      "wet_seasonyes:insecticide_sprayYes"
    ),
    prob = 0.95, prob_outer = 0.99
  ) +
    scale_y_discrete(labels = c(
      "num_toilets_in_home:wet_seasonyes" = "Toilets-int", 
      "num_domestic_water_containers_in_home:wet_seasonyes" = "Water-int",
      "wet_seasonyes:larvicide_applied_to_waterYes" = "Larvicide-int",
      "wet_seasonyes:insecticide_sprayYes" = "Insecticide-int"
    ))
...
if(params$rerun_fit_young2_bayes) {
  fit_young2_bayes <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented3 + in_school + distance_to_river_scaled +
      num_toilets_in_home + num_domestic_water_containers_in_home + 
      num_toilets_in_home:wet_season + num_domestic_water_containers_in_home:wet_season + 
      insecticide_spray + larvicide_applied_to_water + 
      insecticide_spray:wet_season + larvicide_applied_to_water:wet_season + 
      NFI250_home + EVI250_home + 
      wet_season + visit_year,
    data = aedes_red_young,
    seed = seed_val
  ...
  saveRDS(fit_young2_bayes, file = here("output", "fit-young2-bayes.rds"))
  
} else { 
  fit_young2_bayes <- readRDS(file = here("output", "fit-young2-bayes.rds")) 
}
```


\blandscape
Young model with interactions (with visit 5)
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_young2_bayes)
```
\elandscape
...
gen_post_plots <- function(fit){
  ...
  fit_post_area4 <- mcmc_areas(fit, regex_pars = c("NFI250", "EVI250", "year"),
    prob = 0.95, prob_outer = 0.99)
...
if(params$rerun_fit_school1) {
  fit_school1 <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
  ...
  saveRDS(fit_school1, file = here("output", "fit-school1.rds"))
  
} else { 
  fit_school1 <- readRDS(file = here("output", "fit-school1.rds")) 
}
```
...
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_school1)
```
\elandscape

```{r fit-school1-no-visit5}
aedes_red_school_no5 <- aedes_red[(aedes_red$in_school == "yes") & 
                                  (aedes_red$visit != "visit5"), ]

if(params$rerun_fit_school1_no5) {
  fit_school1_no_visit5 <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented7  + distance_to_river_scaled +
      num_toilets_in_home + num_domestic_water_containers_in_home + 
      insecticide_spray + larvicide_applied_to_water + 
      ...
      NFI250_home + EVI250_home + 
      wet_season + visit_year,
    data = aedes_red_school_no5,
    seed = seed_val
  )
  
  saveRDS(fit_school1_no_visit5, file = here("output", "fit-school1-no-visit5.rds"))
  
} else { 
  fit_school1_no_visit5 <- readRDS(file = here("output", "fit-school1-no-visit5.rds")) 
}
```


\blandscape
School model without interactions (without visit 5)
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_school1_no_visit5)
```
\elandscape
...
gen_post_plots <- function(fit){
  ...
  fit_post_area3 <- mcmc_areas(
    ...
    regex_pars = c(
      "num_toilets_in_home:wet_seasonyes", 
      "num_domestic_water_containers_in_home:wet_season",
      "insecticide_sprayYes:wet_seasonyes",
      "larvicide_applied_to_waterYes:wet_seasonyes"
    ),
    prob = 0.95, prob_outer = 0.99) +
    scale_y_discrete(labels = c(
      "num_toilets_in_home:wet_seasonyes" = "Toilets-int", 
      "num_domestic_water_containers_in_home:wet_seasonyes" = "Water-int",
      "insecticide_sprayYes:wet_seasonyes" = "Insecticide-int",
      "larvicide_applied_to_waterYes:wet_seasonyes" = "Larvicide-int"
    ))
...
if(params$rerun_fit_school2) {
  fit_school2 <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented7  + distance_to_river_scaled +
      num_toilets_in_home + num_domestic_water_containers_in_home + 
      insecticide_spray + larvicide_applied_to_water + 
  ...
  saveRDS(fit_school2, file = here("output", "fit-school2.rds"))
  
} else { 
  fit_school2 <- readRDS(file = here("output", "fit-school2.rds")) 
}
```


\blandscape
School model with interactions (with visit 5)
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_school2)
```
\elandscape


## Models with larval count  
...
gen_post_plots <- function(fit){
  ...
  fit_post_area3 <- mcmc_areas(
    fit,
    regex_pars = "larvae_cont_ind",
    prob = 0.95, prob_outer = 0.99
  ) +
    scale_y_discrete(labels = c("larvae_cont_ind" = "Larvae containter"))
  
  fit_post_area4 <- mcmc_areas(
    fit, 
    regex_pars = c("NFI250_home", "EVI250_home"),
    prob = 0.95, prob_outer = 0.99
  ) + 
    scale_y_discrete(labels = c(
      "NFI250_home", 
...
Model fit: Young children with larval counts and visit 5
Updated model for log saliva ELISA value of individual $i$ at measurement $j$ ($i=1,...,726$, $j=1,...,5$), for Cambodian children ages 2-4.
\begin{equation*}
	\begin{split}
		\log(\text{Saliva ELISA value})_{ij} &= b_{0i} + \beta_{0} + \beta_{\text{Male}}\text{Male}_i \\
...
if(params$rerun_fit_young3) {
  fit_young3 <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented3 + in_school + distance_to_river_scaled +
      num_toilets_in_home + num_domestic_water_containers_in_home + 
      insecticide_spray + larvicide_applied_to_water + larvae_cont_ind +
      NFI250_home + EVI250_home + 
      wet_season + visit_year,
    data = aedes_red_young,
    seed = seed_val
  ...
  saveRDS(fit_young3, file = here("output", "fit-young3.rds"))
  
} else { 
  fit_young3 <- readRDS(file = here("output", "fit-young3.rds")) 
}
```

\blandscape
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_young3)
```
\elandscape


Model fit: School children with larval counts and visit 5  
Updated model for log saliva ELISA value of individual $i$ at measurement $j$ ($i=1,...,726$, $j=1,...,5$), for Cambodian school children (all ages).
\begin{equation*}
	\begin{split}
		\log(\text{Saliva ELISA value})_{ij} &= b_{0i} + \beta_{0} + \beta_{\text{Male}}\text{Male}_i \\
...
gen_post_plots <- function(fit){
  ...
  fit_post_area3 <- mcmc_areas(
    fit, regex_pars = c("larvae"),
    prob = 0.95, prob_outer = 0.99
  ) +
    scale_y_discrete(labels = c(
      "larvae_cont_ind" = "Larvae containter"
    ))
  
  fit_post_area4 <- mcmc_areas(
    fit, regex_pars = c("NFI250", "EVI250"),
    prob = 0.95, prob_outer = 0.99
  )
...
if(params$rerun_fit_school3) {
  fit_school3 <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented7  + distance_to_river_scaled +
      num_toilets_in_home + num_domestic_water_containers_in_home + 
      insecticide_spray + larvicide_applied_to_water + larvae_cont_ind + 
  ...
  saveRDS(fit_school3, file = here("output", "fit-school3.rds"))
  
} else { 
  fit_school3 <- readRDS(file = here("output", "fit-school3.rds")) 
}
```

\blandscape
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_school3)
```
\elandscape


## Model without school predictors
...
if(params$fit_main1) {
  fit_main1 <- stan_gamm4(
    formula = log_saliva_elisa ~ gender + 
      ...
      NFI250_home + EVI250_home +
      wet_season + visit_year +
      visit5 + s(lat_v_lon_s, bs = "tp"),
    random = ~ (1 | study_id),
    data = aedes_red,
    seed = seed_val, 
    iter = 4000, 
    prior = normal(
      location = c(rep(0, 12), 0.1),
      scale = c(rep(2.5, 12), 0.05)
    )
  )
  
  saveRDS(fit_main1, file = here("output", "fit-main1.rds"))
  
} else { 
  fit_main1 <- readRDS(file = here("output", "fit-main1.rds")) 
}
```


```{r, eval = FALSE}
mcmc_trace(fit_main1, regex_pars = c("gender", "age", "river", "toilets"))

mcmc_trace(fit_main1, regex_pars = c("water", "insecticide", "larvicide", "larvae"))

mcmc_trace(fit_main1, regex_pars = c("NFI", "EVI"))

mcmc_trace(fit_main1, regex_pars = c("season", "visit"))
```

\blandscape
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_main1)
```

```{r}
plot_nonlinear(fit_main1, prob = 0.95) +
...
\newpage
## Simple spatiotemporal model
```{r}
if(params$fit_simple) {
  fit_simple <- stan_gamm4(
    formula = log_saliva_elisa ~ wet_season + visit5 + s(lat_v_lon_s, bs = "tp"),
    random = ~ (1 | study_id),
    data = aedes_red,
    seed = seed_val, 
    iter = 4000, 
    prior = normal(
      location = c(rep(0, 1), 0.1),
      scale = c(rep(2.5, 1), 0.05)
    )
  )
  
  saveRDS(fit_simple, file = here("output", "fit_simple.rds"))
  
} else { 
  fit_simple <- readRDS(file = here("output", "fit_simple.rds")) 
}
```
...
mcmc_areas(
  fit_simple, 
  regex_pars = c("wet_season", "visit5"),
  prob = 0.95,
  prob_outer = 0.99
)
```
...
```{r}
plot_nonlinear(fit_simple, prob = 0.95) +
  geom_hline(yintercept = 0, linetype = "dashed")
```
