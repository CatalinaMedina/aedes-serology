...
if(params$fit_main1) {
  ...
  saveRDS(fit_main1_te, file = here("output", "fit-main1_te.rds"))
  
} else { 
  fit_main1_te <- readRDS(file = here("output", "fit-main1_te.rds")) 
}
```


```{r, eval = FALSE}
mcmc_trace(fit_main1_te, regex_pars = c("gender", "age", "river", "toilets"))

mcmc_trace(fit_main1_te, regex_pars = c("water", "insecticide", "larvicide", "larvae"))

mcmc_trace(fit_main1_te, regex_pars = c("NFI", "NVI"))

mcmc_trace(fit_main1_te, regex_pars = c("season", "visit"))
```

\blandscape
```{r, fig.width=10, fig.height=7.5}
gen_post_plots(fit_main1_te)
```

```{r}
plot_nonlinear(fit_main1_te, prob = 0.95) +
...
if(params$fit_main1) {
  ...
  saveRDS(fit_main1_te_y, file = here("output", "fit_main1_te_y.rds"))
  
} else { 
  fit_main1_te_y <- readRDS(file = here("output", "fit_main1_te_y.rds")) 
}
```