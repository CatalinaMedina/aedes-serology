...
```{r process-spatial-data-function}
process_spatial_data <- function(df_og, sat_var_names, sat_var_col_start){
  ...
  col_names <- colnames(df_og[, sat_cols])
  
  var_type <- str_sub(col_names, start = 1, end = 3)
  new_df[, "var_type"] <- rep(var_type, nrow(df_og))
  
  year_seq <- paste0("20", str_sub(col_names, start = 5, end = 6))
  days_seq <- str_sub(col_names, start = 8, end = 10)
  date_seq <- as.Date(paste0(year_seq, "-01-01")) + days(as.numeric(days_seq) - 1)
  new_df[, "date"] <- rep(date_seq, nrow(df_og))
  
  for(i in 1:nrow(df_og)) {
    rows_for_id <- new_row:(new_row + n_sat_cols - 1)
    ...
    new_df[rows_for_id, "sat_var_value"] <- as.numeric(df_og[i, sat_cols])

    new_row <- new_row + n_sat_cols
  }
...
if (params$rerun_data) {
  ...
  saveRDS(aedes_red, file = here("data", "processed_aedes_data"))
  
} else { 
  aedes_red <- readRDS(file = here("data", "processed_aedes_data")) 
}
