...
```{r read-process-spatial-data}
# what column do the environmental variables start?
sat_var_col_start <- 7
sat_var_names <- c("EVI", "NVI", "NFI")
...
new_df <- data.frame(matrix(
  NA, 
  nrow = nrow(df_og) * num_vars,
  ncol = length(non_sat_cols) + length(sat_cols) / num_vars + 1
))

colnames(new_df)[non_sat_cols] <- colnames(df_og)[non_sat_cols]
colnames(new_df)[sat_cols[1]] <- "var_type"


col_types <- rep(NA, ncol(df_og))

for(k in 1:num_vars) {
  col_types[str_detect(colnames(df_og), sat_var_names[k])] <- sat_var_names[k]
...
for(i in 1:nrow(df_og)) {
  for(k in 0:(num_vars - 1)) {
    new_df[new_row + k, non_sat_cols] <- data.frame(df_og[i, non_sat_cols])
  }

  
  
  new_row <- new_row + num_vars
}
```
