...
gen_post_plots <- function(fit){
  ...
  fit_post_area3 <- mcmc_areas(
    fit, regex_pars = c("land", "EVI250_home"),
    prob = 0.95, prob_outer = 0.99
  )
...
if(params$rerun_fit_young) {
  ...
  saveRDS(fit_young_flood32, file = here("output", "fit_young_flood32.rds"))
  
} else { 
  fit_young_flood32 <- readRDS(file = here("output", "fit_young_flood32.rds")) 
}
```
...
gen_post_plots(fit_young_flood32)
```
\elandscape
...
if(params$rerun_fit_school) {
  ...
  saveRDS(fit_school_flood32, file = here("output", "fit_school_flood32.rds"))
  
} else { 
  fit_school_flood32 <- readRDS(file = here("output", "fit_school_flood32.rds")) 
}
```
...
gen_post_plots(fit_school_flood32)
```
\elandscape
...
```{r fit-main2}
if(params$fit_main2) {
  fit_main2_16 <- stan_gamm4(
    formula = log_saliva_elisa ~ gender + 
      age_cented7  + distance_to_river_scaled +
      modal_land_type +
      num_toilets_in_home + num_domestic_water_containers_in_home +
      insecticide_spray + larvicide_applied_to_water  +
      NFI250_home16 + NFI250_home +
      wet_season + visit_year +
      visit5 + s(lat_s, lon_s, bs = "tp"),
    random = ~ (1 | study_id),
    data = aedes_red,
  ...
  saveRDS(fit_main2_16and32, file = here("output", "fit_main2_16and32.rds"))
  
} else { 
  fit_main2_16and32 <- readRDS(file = here("output", "fit_main2_16and32.rds")) 
}
```
...
if(params$fit_main2) {
  ...
  saveRDS(fit_main2_48, file = here("output", "fit_main2_48.rds"))
  
} else { 
  fit_main2_48 <- readRDS(file = here("output", "fit_main2_48.rds")) 
}
```
...
if(params$fit_main2) {
  ...
  saveRDS(fit_main2_16, file = here("output", "fit_main2_16.rds"))
  
  
   fit_main2_32 <- stan_gamm4(
    formula = log_saliva_elisa ~ gender + 
      age_cented7  + distance_to_river_scaled +
      modal_land_type +
      num_toilets_in_home + num_domestic_water_containers_in_home +
  ...
  saveRDS(fit_main2_32, file = here("output", "fit_main2_32.rds"))
  
} else { 
  fit_main2_16 <- readRDS(file = here("output", "fit_main2_16.rds")) 
  fit_main2_32 <- readRDS(file = here("output", "fit_main2_32.rds")) 
}
```
...
```{r fit-main6}
if(params$fit_main2) {
  fit_main2_32flood_16veg <- stan_gamm4(
    formula = log_saliva_elisa ~ gender + 
      age_cented7  + distance_to_river_scaled +
      modal_land_type +
      num_toilets_in_home + num_domestic_water_containers_in_home +
      insecticide_spray + larvicide_applied_to_water  +
      EVI250_home16 + NFI250_home +
      wet_season + visit_year +
      visit5 + s(lat_s, lon_s, bs = "tp"),
    random = ~ (1 | study_id),
    data = aedes_red,
  ...
  saveRDS(fit_main2_32flood_16veg, file = here("output", "fit_main2_32flood_16veg.rds"))
  
} else { 
  fit_main2_32flood_16veg <- readRDS(file = here("output", "fit_main2_32flood_16veg.rds")) 
}
```
...
gen_post_plots(fit_main2_16)
gen_post_plots(fit_main2_32)
gen_post_plots(fit_main2_48)
gen_post_plots(fit_main2_16and32)
gen_post_plots(fit_main2_32flood_16veg)
```
...
```{r display-visit-exposure-surface, fig.width=7.5, fig.asp=1, fig.align="center"}
...
if(params$rerun_simple_surf){
  spdf <- SpatialPointsDataFrame(
    ...
    data = select(aedes_red, log_saliva_elisa, time, visit5)
  )
  
  time_surf <- glgm(
    formula = log_saliva_elisa ~ time + visit5,
    data = spdf,
    grid = 75
  )
...
dry_season_surf$parameters$summary %>% select(mean, "0.025quant", "0.975quant") %>% round(3)
```
