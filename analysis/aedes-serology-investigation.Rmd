...
aedes_survey_cleaned <- aedes_survey_og %>% 
  ...
  mutate(how_often_use_bed_net = factor(
    ...
    levels = c("Never", "Rarely", "Regularly/ Most of the time", "All of the time")
  )) %>% 
...
```{r elisa-by-age-mapped, fig.asp=0.8}
...
```{r elisa-by-num_domestic_water_containers_in_home}
aedes_red_ind %>% 
...
```{r elisa-by-num_toilets_in_home}
aedes_red_ind %>% 
...
```{r elisa-by-how_often_use_bed_net}
aedes_red_ind %>% 
...
```{r elisa-by-larvicide_applied_to_water}
aedes_red_ind %>% 
...
```{r elisa-by-how_often_mosquito_coils_burned}
aedes_red_ind %>% 
