...
v1 <- filter(aedes_red, visit == "visit1")

v1_spdf <- SpatialPointsDataFrame(
  coords = select(v1, lon, lat),
  data = select(v1, log_saliva_elisa)
)

v1_likfit <- likfitLgm(formula = log_saliva_elisa ~ 1, data = aedes_v1_spdf) # doesn't work

v1_krige_fit <- krigeLgm(
...
all_spdf <- SpatialPointsDataFrame(
  coords = select(aedes_red, lon, lat),
  data = select(aedes_red, log_saliva_elisa, wet_season)
)
...
lgm(
  ...
  data = all_spdf,
  grid = 50,
  covariates = # The spatial covariates used in prediction, either a raster stack or
  #list of rasters. Covariates in formula but not in data will be extracted from covariates.
)
```
