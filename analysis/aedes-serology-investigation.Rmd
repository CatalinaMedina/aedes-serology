...
gen_post_plots <- function(fit){
  ...
  fit_post_area2 <- mcmc_areas(
    ...
    scale_y_discrete(labels = c(
      ...
      "close_to_riveryes" = "River close(yes)"
    ))
...
if(params$rerun_fit_young1_bayes) {
  fit_young1_bayes <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented3 + in_school + close_to_river +
...
Fit :  
Updated model for log saliva ELISA value of individual $i$ at measurement $j$ ($i=1,...,726$, $j=1,...,5$), for Cambodian school children ages 5-9.
\begin{equation*}
	\begin{split}
		\log(\text{Saliva ELISA value})_{ij} &= b_{0i} + \beta_{0} + \beta_{\text{Male}}\text{Male}_i \\
...
```{r fit-school1}
aedes_red_school <- aedes_red[aedes_red$in_school == "yes" & aedes_red$age >= 5,]

if(params$rerun_fit_school1) {
  fit_school1 <- stan_lmer(
    formula = log_saliva_elisa ~ (1 | study_id) + 1 + gender + 
      age_cented7  + close_to_river +
      num_toilets_in_home + num_domestic_water_containers_in_home + 
      insecticide_spray + larvicide_applied_to_water + 
