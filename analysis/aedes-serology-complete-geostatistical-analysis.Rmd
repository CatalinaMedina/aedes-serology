...
```{r Examine spatial parameters, eval = FALSE}
all_data <- main_data
formula <- main_formula
seed <- 1234
...
#Variance: Variation in the spatial field
print("variance")
variance_sum <- inla.zmarginal(spde.est$marginals.variance.nominal[[1]])

plot(
  spde.est[["marginals.variance.nominal"]][[1]], 
  type = "l",
  main = "Nominal variance, posterior density",
  xlab = "Nominal variance variance",
  ylab = "Density"
)
abline(v = variance_sum$quant0.025, lty = 2)
abline(v = variance_sum$quant0.975, lty = 2)

#Range: Typical distance between high and low regions
# The range of the process is the distance such that the correlation between values is close to 0.1
print("range") 
range_sum <- inla.zmarginal(spde.est$marginals.range.nominal[[1]])


plot(
  spde.est[["marginals.range.nominal"]][[1]], 
  type = "l",
  main = "Nominal range, posterior density",
  xlab = "Nominal range: ie the distance that we have correlation equals 0.1 (meters)",
  ylab = "Density"
)
abline(v = range_sum$quant0.025, lty = 2)
abline(v = range_sum$quant0.975, lty = 2)

library(gstat)
res_tab <- data.frame(
  "lon" = all_data$lon,
  "lat" = all_data$lat,
  "log_saliva_elisa" = all_data$log_saliva_elisa,
  "mu" = main_fit$summary.fitted.values$mean,
  "sigma2" = main_fit$summary.fitted.values$mean ^ 2 / main_fit$summary.hyperpar[1, "mean"]
) %>% mutate(
  pearson_iid = (log_saliva_elisa - mu) / sqrt(sigma2)
)
...
all_results <- main_fit_df %>% 
  ...
  mutate(per_change_lb = 100 * (exp(quant025) - 1)) %>% 
  mutate(per_change_mean = 100 * (exp(mean) - 1)) %>% 
  mutate(per_change_ub = 100 * (exp(quant975) - 1)) 
```
...
```{r Produce forest plot with percent change}
alpha <- 0.8

dodge <- 0.8

min_val <- min(
  all_results$per_change_lb[all_results$display_names != "Flooding index (NFI)"],
  na.rm = TRUE
)
max_val <- max(
  max(
    all_results$per_change_ub[all_results$display_names != "Flooding index (NFI)"],
    na.rm = TRUE
  ),
  0.6
)
...
all_results %>% 
  ggplot(aes(x = per_change_mean , y = display_names, color = Model)) +
    geom_point(
      alpha = alpha, 
      position = position_dodge2(width = dodge, reverse = TRUE)
    ) +
    geom_linerange(
      aes(xmin = per_change_lb, xmax = per_change_ub), 
      alpha = alpha, 
      position = position_dodge2(width = dodge, reverse = TRUE)
    ) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    ...
    xlab("Estimated percent change in exposure") +
    ylab("Covariate") +
  scale_color_manual(values = c("#440154FF", "#31688EFF", "#35B779FF")) +
  #scale_color_manual(values = c("#3F3994FF", "#7A0403FF", "#2DB17EFF")) +
  coord_cartesian(xlim=c(min_val, max_val))
...
```{r Prepare estimates table with percent change in exposure}
main_fit_df <- read_csv(
  here("output", "main_fit_df.csv"),
  show_col_types = FALSE
) %>% 
  ...
  mutate(per_change_ub = 100 * (exp(quant975) - 1)) %>% 
  mutate(Main = paste0(
    round(per_change_mean, 2), 
    " (", round(per_change_lb, 2), ", ", round(per_change_ub, 2), ")"
  )) %>% 
    select(pred_names, Main) 
...
```{r Produce estimates table with percent change in exposure}
all_results %>% 
  select(display_names, Main, Larval, Adult) %>% 
kbl(
  #booktabs = TRUE, 
...
```{r}
get_sample_variogram <- function(v){
  pagodas_v <- filter(pagodas, visit == paste0("visit", v))
  coordinates(pagodas_v) <-  data.frame(
    x = pagodas_v$x, 
    y = pagodas_v$y
  )
  
  variogram_int_v <- variogram(
    log_saliva_elisa ~ 1, 
    data = pagodas_v,
    cutoff = 2000,
    width = 40
  ...
  variogram_preds_v <- variogram(
    log_saliva_elisa ~ 1 + gender + age_cented7 + insecticide_spray + larvicide_applied_to_water + num_toilets_in_home + num_domestic_water_containers_in_home + modal_land_type + distance_to_river_scaled + NFI250_home, 
    data = pagodas_v,
    cutoff = 2000,
    width = 40
  )
  ...
  data.frame(
    Estimate = c(variogram_int_v$gamma, variogram_preds_v$gamma),
    Distance = c(variogram_int_v$dist, variogram_preds_v$dist),
    Model = c(
      rep("Intercept only", length(variogram_int_v$dist)),
      rep("Full model", length(variogram_preds_v$dist))
    ),
    Visit = paste0("Visit ", v)
  ) 
}

sample_variogram <- rbind(
  get_sample_variogram(v = "1"),
  get_sample_variogram(v = "2"),
  get_sample_variogram(v = "3"),
  get_sample_variogram(v = "4")
) %>% 
  mutate(Model = factor(
    Model, 
    levels = c("Intercept only", "Full model")
  )) 
...
ggplot(aes(x = Distance, y = Estimate)) +
  ...
  geom_line() +
  ylim(c(0, 0.225)) +
  facet_grid(Visit ~ Model) +
  theme_minimal(base_size = 15) +
  xlab("Distance (meters)") +
  ylab("Semivariance") +
  ggtitle("Empirical Variogram by Visit")
```



## Investigation of entomological indicies
```{r Correlation of entomological indicies}
library(corrplot)

indicies <- larval_house %>% 
  select(
...
```{r Fit larval model with ave pci}
if (params$refit_larval_model) {
  pred_rownames_ordered <- c(
    "(Intercept)",
    "genderMale",
  ...
  # Extract relevant data
  data_with_ave_pci <- larval_house %>% 
    select(
      time, 
      lon = x, lat = y, 
      log_saliva_elisa,
  ...
  # Specify model formula, 'f()' incorporates spatial effect and replicate 
  formula_with_ave_pci <- y ~ #0 + Intercept
    gender + 
    age_cented7  + distance_to_river_scaled +
    modal_land_type +
    num_toilets_in_home + num_domestic_water_containers_in_home +
  ...
  larval_fit_pci <- fit_multi_visit_inla_model(
    all_data = data_with_ave_pci, 
    formula = formula_with_ave_pci,
    seed = 1234
  )
  
  larval_fit_pci_df <- larval_fit_pci$summary.fixed %>% 
    mutate(unordered_names = rownames(larval_fit_pci$summary.fixed)) %>% 
    mutate(pred_names = factor(
      unordered_names,
      levels = c(pred_rownames_ordered, "ave_pci"),
      labels = c(pred_names_vec, "Average PCI")
    )) %>% 
    arrange(pred_names) %>% 
    select(quant025 = "0.025quant", mean = "mean", quant975 = "0.975quant", pred_names) 
  
  write_csv(larval_fit_pci_df, here("output", "larval_fit_pci_df.csv"))
}
```

```{r Prepare larval pci output for forest plot}

larval_fit_pci_df <- read_csv(
  here("output", "larval_fit_pci_df.csv"),
  show_col_types = FALSE
)
...
cleaned_results <- larval_fit_pci_df %>% 
  filter(pred_names != "(Intercept)") %>% 
  add_row(pred_names = "Sex [ref: Female]") %>% # Sex [ref: Female]
  add_row(pred_names = "Insecticide used [ref: No]") %>% # Insecticide used [ref: No]
  add_row(pred_names = "Larvicide used [ref: No]") %>% # Larvicide used [ref: No]
  add_row(pred_names = "Modal land type [ref: Urban]") %>% # Modal land type [ref: Urban]
  add_row(pred_names = "Wet season [ref: No]") %>% # Wet season [ref: No]
  mutate(display_names = factor(
    pred_names, 
    levels = rev(pred_names_vec), 
    labels = rev(display_names_vec) 
...
```{r Produce larval model with pci forest plot}
alpha <- 0.8

dodge <- 0.8

min_val <- min(
  cleaned_results$quant025[all_results$display_names != "Flooding index (NFI)"],
  na.rm = TRUE
)
max_val <- max(
  max(
    cleaned_results$quant975[all_results$display_names != "Flooding index (NFI)"],
    na.rm = TRUE
  ),
  0.6
)
...
cleaned_results %>% 
  ggplot(aes(x = mean, y = display_names)) +
    geom_point(
      alpha = alpha, 
      position = position_dodge2(width = dodge, reverse = TRUE)
    ) +
...
```{r Produce larval model with pci forest plot with percent change in exposure}
alpha <- 0.8

dodge <- 0.8

min_val <- min(
  cleaned_results$per_change_lb[all_results$display_names != "Flooding index (NFI)"],
  na.rm = TRUE
)
max_val <- max(
  max(
    cleaned_results$per_change_ub[all_results$display_names != "Flooding index (NFI)"],
    na.rm = TRUE
  ),
  0.6
)
...
cleaned_results %>% 
  ggplot(aes(x = per_change_mean, y = display_names)) +
    geom_point(
      alpha = alpha, 
      position = position_dodge2(width = dodge, reverse = TRUE)
    ) +
