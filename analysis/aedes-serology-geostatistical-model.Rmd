...
```{r larval-date-prep}
aedes_larval_matching_data <- aedes_red %>% 
  filter(visit != "visit5") %>% 
  mutate(visit = droplevels(visit))
  ...
  ##############################################################################
  # Check how much this would subset the data
  ##############################################################################
  
  larval_nn <- nn2(
    select(.data = larval_hs, X, Y),
    query = select(.data = aedes_larval_matching_data, x, y),
    k = 1,
    radius = 300
  )
  
  
  aedes_larval <- aedes_larval_matching_data %>% 
...
prep_and_fit_visit_surf_model <- function(visit_name, data = aedes_red){
  ## extract relevant data (not sure if sorting is necessary)
  coords_st <- data %>% 
    arrange(study_id) %>% 
...
if(params$refit_v1_fit) {
  v1_fit <- prep_and_fit_visit_surf_model(visit_name = "visit1")
...
if(params$refit_v2_fit) {
  v2_fit <- prep_and_fit_visit_surf_model(visit_name = "visit2")
...
if(params$refit_v3_fit) {
  v3_fit <- prep_and_fit_visit_surf_model(visit_name = "visit3")
...
if(params$refit_v4_fit) {
  v4_fit <- prep_and_fit_visit_surf_model(visit_name = "visit4")
...
if(params$refit_v5_fit) {
  v5_fit <- prep_and_fit_visit_surf_model(visit_name = "visit5")
...
## extract relevant data (not sure if sorting is necessary)
  ...
  ### For prediction: 
  ## This is with the mesh (likely too complicated/resolution too high for prediction)
...
visit_fit$summary.fitted.values[pred_index, c(1:3, 5)]
```
