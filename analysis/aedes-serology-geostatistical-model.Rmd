...
main_fit_wov5$summary.fixed %>% 
  select(quant025 = "0.025quant", mean = "mean", quant975 = "0.975quant") %>% 
  mutate(names = rownames(main_fit_wov5$summary.fixed)) %>% 
...
train_stack <- inla.stack(
  data = list(y = ad_train$adult_count),
  A = list(train_A, 1),
  effects = list(
    s.index,
