...
lat_org <- ind_visit_surf_df %>% 
  filter(visit == "Visit 1" & type == "Mean")

# 
# lat_n <- nrow(lat_org)
# lat_n5 <- round(lat_n / 6, digits = 0)
#   
# lat_breaks <- lat_org$y_m[c(1 * lat_n5, 3 * lat_n5, 5 * lat_n5)]
# lat_labels <- round(lat_org$lat[c(1 * lat_n5, 3 * lat_n5, 5 * lat_n5)], 2)

lat_breaks <- c(1266000, 1267000, 1268000)
lat_labels <- c(11.452, 11.461, 11.47)


pdf(here("output", "visit-prediction-surfaces-w-v5.pdf"), width = 10)

ggplot(ind_visit_surf_df) +
  geom_raster(mapping = aes(x = x_m, y = y_m, fill = Prediction)) +
  facet_grid(type ~ visit) +
  xlim(c(445000, 453000)) + 
  ylim(c(1265000, 1269000)) +
  coord_equal() +
  scale_fill_viridis() +
  theme_bw() +
  theme(
...
ind_visit_surf_df2 <- ind_visit_surf_df %>% 
...
ggplot() +
  geom_raster(data = ind_visit_surf_df2, aes(x = lon, y = lat, fill = Prediction)) +
  facet_grid(type ~ visit) +
  coord_equal() +
  scale_fill_viridis() +
  theme_classic() +
