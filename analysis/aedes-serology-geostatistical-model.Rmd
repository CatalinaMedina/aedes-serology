...
if (params$rerun_data) {
  ...
  larval_hs <- read_xlsx(here("data", "House Survey Data_Extracted 03May2021.xlsx")) %>% 
    ...
    mutate(water_containers = as.numeric(water_containers)) %>% 
    drop_na() %>% 
...
```{r}
aedes_red_sp <- aedes_red
coordinates(aedes_red_sp) <- c("lon", "lat")
proj4string(aedes_red_sp) <- "+proj=longlat"
...
formula <- log_saliva_elisa ~ field(cbind(lon, lat), model = spde, group = time)
```
