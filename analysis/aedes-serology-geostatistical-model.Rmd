...
if(params$refit_vistit_fit) {
  ...
  saveRDS(v1_fit, here("output", "v1-fit.rds"))
  saveRDS(v2_fit, here("output", "v2-fit.rds"))
  saveRDS(v3_fit, here("output", "v3-fit.rds"))
  saveRDS(v4_fit, here("output", "v4-fit.rds"))
  saveRDS(v5_fit, here("output", "v5-fit.rds"))
  
} else {
  
  v1_fit <- readRDS(here("output", "v1-fit.rds"))
  v2_fit <- readRDS(here("output", "v2-fit.rds"))
  v3_fit <- readRDS(here("output", "v3-fit.rds"))
  v4_fit <- readRDS(here("output", "v4-fit.rds"))
  v5_fit <- readRDS(here("output", "v5-fit.rds"))
...
ggplot() +
  geom_raster(data = ind_vist_surf_df, aes(x = lon, y = lat, fill = Prediction)) +
...
ind_vist_surf_df2 <- ind_vist_surf_df %>% 
...
ggplot() +
  geom_raster(data = ind_vist_surf_df2, aes(x = lon, y = lat, fill = Prediction)) +
  facet_grid(type ~ visit) +
  coord_equal() +
  scale_fill_viridis() +
  theme_classic() +
...
aedes_larval_red_og <- aedes_red %>% 
  ...
  match_larval_data <- function(visit_num){
    ...
    aedes_larval_red$water_larval_ind <- rep(NA, nrow(aedes_larval_red))
    aedes_larval_red$water_larval_ind[matched] <- larval_hs_red$container_index[larval_nn$nn.idx[matched]]
...
ggplot(aedes_red_w_larval, aes(x = x, y = y, color = water_larval_ind)) +
...
aedes_red_w_larval %>% 
  filter(!is.na(water_larval_ind)) %>% 
ggplot(aes(x = x, y = y, color = water_larval_ind)) +
  geom_point(alpha = 0.25) +
  scale_color_viridis(direction = -1) +
  xlim(range(aedes_red_w_larval$x)) +
  ylim(range(aedes_red_w_larval$y)) +
...
larval_hs %>% 
  mutate(water_larval_ind = container_index) %>% 
ggplot(aes(x = larval_X, y = larval_Y, color = water_larval_ind)) +
  geom_point(alpha = 0.25) +
  scale_color_viridis(direction = -1) +
  xlim(range(aedes_red_w_larval$x)) +
  ylim(range(aedes_red_w_larval$y)) +
...
```{r forest_plot_with_intercept}
larval_fit$summary.fixed %>% 
  select(quant025 = "0.025quant", median = "0.5quant", quant975 = "0.975quant") %>% 
  cbind(names = rownames(larval_fit$summary.fixed)) %>% 
  filter(names != "(Intercept)") %>% 
  ggplot(aes(x = median, y = names)) +
    geom_point() +
    geom_linerange(aes(xmin = quant025, xmax = quant975)) +
